...
<h2>Summary of mortgages</h2>

<?php session_start();
//MORTGAGE 1
$price = floatval(preg_replace("/[^0-9.]*/","",$_POST["price_1"]));
$down = floatval(preg_replace("/[^0-9.]*/","",$_POST["down_1"]));
$principal = $price * (1 - $down/100);
$length = floatval(preg_replace("/[^0-9.]*/","",$_POST["length_1"])) * 12;
...
$down_1 = floatval(preg_replace("/[^0-9.]*/","",$_POST["down"]));
$principal_1 = $price_1 * (1 - $down_1/100);
$length_1 = floatval(preg_replace("/[^0-9.]*/","",$_POST["length"])) * 12;
...
"<tr class='hd'><th width='110'></th><th width='120'>Mortgage 1<br />(M-1)</th><th width='120'>Mortgage 2<br />(M-2)</th></tr>".
"<tr class='odd'><td class='lt'>Price</td><td>".number_format($price_1,2,'.',',')."</td><td>".number_format($price,2,'.',',')."</td></tr>".
"<tr class='even'><td class='lt'>Down</td><td>" . $down_1 . "%</td><td>" . $down . "%</td></tr>".
"<tr class='odd'><td class='lt'>Principal</td><td>$" . number_format($principal_1,2,'.',','). "</td><td>$" . number_format($principal,2,'.',','). "</td></tr>".
...
"<tr class='odd'><td class='lt'>Total Interest</td><td>$" . number_format($mpayment_1*$length_1-$principal_1,2,'.',',')."</td><td>$" . number_format($mpayment*$length-$principal,2,'.',',')."</td></tr>".
"</table></td></tr></table>";
echo $summary;
...
<!--  MORTGAGE 1 -->
<?php
	echo "<h3 class='expand1'>Mortgage 1</h3><br />";
	echo "<div class='expand-div-1'>";

for ($i=1; $i<=$length_1/12; $i++){
	$output_1 ="";
	$year_interest_1 = 0;
	$year_principal_1 = 0;
	$year_total_1 = 0;
...
<?php
	echo "<h3 class='expand2'>Mortgage 2</h3><br />";
...
//chart 1 line
		var chart;
			chart = new Highcharts.Chart({
				...
				series: [{
					name: 'M-2 Monthly Interest',
					data: monthly_interest

				}, {
					name: 'M-2 Monthly Principal',
					data: monthly_principal
				},{
					name: 'M-1 Monthly Interest',
					data: monthly_interest_1

				}, {
					name: 'M-1 Monthly Principal',
...
//chart 2 PIE
		var chart2;
			chart2 = new Highcharts.Chart({
				...
				xAxis: {
				    categories: ['Mortgage 1', 'Mortgage 2']
				},
				credits: {
					text: 'DailyCalculators.com',
					href: 'http://www.dailycalculators.com'
