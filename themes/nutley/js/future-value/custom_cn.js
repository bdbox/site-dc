...
function getData(){
	...
	duration = "投资时间跨度: " + ((Math.floor(term/365) > 0 ? (Math.floor(term/365) + "年") : "")) + term%365 + "天。";
...
function doReport(){
	...
	if (term > 0) {
		...
		roiStr += "投资回报率是" + ((deposit*Math.pow((1+rate/compound), compound*term) - deposit)/deposit*100).toFixed(2) + "%";
		roiStr += "<br />年利率(Annual Percentage Rate)是 " + ((Math.pow((1+rate/compound), compound)-1)*100).toFixed(2) + "%";
		roiStr += "<br />未来价值是￥" + addCommas(t_interest) + ".";
