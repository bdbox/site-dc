...
function doReport(mode){
	...
	if (term > 0) {
		...
		if (mode == "target"){
			roiStr += "<strong>投资目标是￥" + $("#txtTarget").val() + "</strong>";
		}else if (mode == "length"){
			roiStr += "<strong>投资长度是￥" + $("#txtTerm").val() + "</strong>";
		}else if (mode == "rate"){
			roiStr += "<strong>回报率是" + $("#txtRate").val() + "</strong>";
		}else if (mode == "regular"){
			roiStr += "<strong>定期追加投资是￥" + $("#txtRegular").val() + "</strong>";
		}else if (mode == "deposit"){
			roiStr += "<strong>初期投资是￥" + $("#txtDeposit").val() + "</strong>";
		}else {
			
		}
		
		roiStr += "<br />总计投资（现值）: ￥" + addCommas((deposit + regular_pay*(regular_term*term)).toFixed(2));
		roiStr += "<br />总计回报: ￥"  +  addCommas((end_deposit[end_deposit.length-1] - (deposit + regular_pay*(regular_term*term))).toFixed(2));
		roiStr += "<br />总计投资（期值）: " + addCommas(end_deposit[circle*term-1].toFixed(2));
		
		roiStr += "<br />投资回报率是" + ((end_deposit[end_deposit.length-1] - (deposit + regular_pay*(regular_term*term)))/(deposit + regular_pay*(regular_term*term))*100).toFixed(2) + "%";
...
function getChartOptions(){
	chartOptions = {
		...
		series: [{
			...
			name: '投资和收益',
			data: [
				['投资',       deposit + regular_pay*(regular_term*term)],
				{
					name: '收益',
